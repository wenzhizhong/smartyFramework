...
{//类定义开始
    ...
    private function save($file)
    {
        ...
        // 如果是图像文件 检测文件格式
        if(isset($file['extension'])&& in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png','swf')) && false === getimagesize($file['tmp_name'])) {
            $this->error = '非法图像文件';
            return false;
        }
        @chmod($filename, 0644);
    ...
    public function upload($savePath ='')
    {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                //登记上传文件的扩展信息
                $file['key']        =  $key;
                $getExt_            =$this->getExt($file['name']);
                isset($getExt_) && $file['extension']  = $getExt_;
    ...
    private function getSaveName($filename)
    {
        ...
        if(empty($rule)) {//没有定义命名规则，则保持文件名不变
            ...
            if(function_exists($rule)) {
                ...
                //使用给定的文件名作为标识号
                $saveName = $rule;
                isset($filename['extension'])&&$saveName = $rule.".".$filename['extension'];
    ...
    private function check($file) {
        ...
        //检查文件类型
        if(isset($file['extension'])&&!$this->checkExt($file['extension'])) {
